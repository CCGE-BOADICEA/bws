...
from bws.cancer import GeneticTest, GENETIC_TESTS, GeneticTests,\
    PathologyTest, PathologyTests
...
class PedigreeTests(TestCase, ErrorTests):
    ...
    def test_mztwin_genetic_tests(self):
        ...
        pedigree = pedigree_file.pedigrees[0]
        m2 = pedigree.get_person_by_name('M2')
        m2.gtests = GeneticTests._make([GeneticTest("S", "P")
                                        if i == 0 else GeneticTest("0", "0")
                                        for i in range(len(GENETIC_TESTS))])
        self._add_twin(pedigree, twin=m2)
        m2.gtests = GeneticTests._make([GeneticTest("S", "N")
                                        if i == 0 else GeneticTest("0", "0")
                                        for i in range(len(GENETIC_TESTS))])
